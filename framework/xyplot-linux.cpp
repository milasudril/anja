...
class XYPlot::Impl
	{
	public:
		Impl(XYPlot& me,EventHandler& handler):r_handler(handler),m_width{0}
			,m_height{0},m_domain_max{0}
			{
	...
	private:
		...
		static void onSizeChange(GtkWidget *widget, GtkAllocation *allocation, void *data)
			{
			Impl* _this=(Impl*)data;
		...
		static gboolean onMouseMove(GtkWidget* object
			...
			if(belongsTo(point,_this->m_domain_current_active))
				{_this->r_handler.mouseMove(point,keymaskFromSystem(event->state));}
			return TRUE;
			}
		...
		static gboolean onMouseDown(GtkWidget* object,GdkEventButton* event, void* impl)
			...
			auto point=_this->toPlotCoords(Curve::Point{event->x,event->y});
			if(belongsTo(point,_this->m_domain_current_active))
				{_this->r_handler.mouseDown(point,keymaskFromSystem(event->state));}
			return TRUE;
			}
		...
		static gboolean onMouseUp(GtkWidget* object,GdkEventButton* event, void* impl)
			...
			auto point=_this->toPlotCoords(Curve::Point{event->x,event->y});
			if(belongsTo(point,_this->m_domain_current_active))
				{_this->r_handler.mouseUp(point,keymaskFromSystem(event->state));}
			return TRUE;
			}
...
XYPlot::XYPlot(GuiContainer& parent,EventHandler& handler)
	{
	m_impl=new Impl(*this,handler);
	m_impl->addTo(parent);
	m_impl->backgroundSet(1);
	}
...
void XYPlot::curveAdd(const Curve& curve)
	{
	m_impl->curveAdd(curve);
	}

void XYPlot::domainSet(const Curve::Domain& domain)
	{
	m_impl->domainSet(domain);
	}

void XYPlot::backgroundSet(bool light)
	{
	m_impl->backgroundSet(light);
	}
